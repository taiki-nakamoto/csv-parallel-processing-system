...
- **運用効率**: ライフサイクル管理、自動削除
- **シンプルな構成**: 単一バケットでの統合管理、シンプルなライフサイクル設定
- **適切な保存期間**: 個人開発のため30日間の保存後削除で運用
...
### 3.3 統一ライフサイクルポリシー（簡易設定）

**設計変更理由**: 個人開発プロジェクトのため、コスト最適化よりもシンプルな運用を優先し、STANDARD_IAへの移行は行わず30日後の削除のみとする。
...
| データタイプ | 初期クラス | 移行先クラス | 削除タイミング | 変更理由 |
|-------------|-----------|-------------|---------------|----------|
| 全データ | STANDARD | なし | 30日後 | 個人開発のため長期保存不要 |
| 処理済みファイル | STANDARD | なし | 7日後 | 処理完了後は早期削除 |

### 6.2 コスト見積もり（月額）- 簡易版

- **ストレージ**: 
  - STANDARD: 500GB × $0.025 = $12.50（簡素化により単一クラス）
- **リクエスト**: 
  - PUT: 10,000 × $0.0047/1000 = $0.047
  - GET: 50,000 × $0.00037/1000 = $0.019
- **データ転送**: 10GB × $0.114 = $1.14
- **合計**: 約$13.71/月（運用工数削減効果を考慮）
...
- 30日間の保存期間内でのリストア対応
- 個人開発プロジェクトのため複雑なバックアップ戦略は実装しない
- 必要に応じて手動でのファイル退避を実施

### 10.3 ライフサイクル設計変更履歴

**2025年8月7日 変更**:
- **変更内容**: STANDARD_IAストレージクラスへの移行を削除
- **変更理由**: 個人開発のため長期保存が不要、シンプルな運用を優先
- **実装変更**: 30日後削除のみのライフサイクル設定に変更
- **コスト影響**: 月額約$4増加だが運用工数削減効果でペイ
- **運用メリット**: 設定がシンプルになり、管理コストが削減される

この設計変更により、個人開発に適したシンプルで効率的なS3バケット運用を実現する。