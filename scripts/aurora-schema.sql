-- ============================================
-- CSV並列処理システム - Aurora PostgreSQL スキーマ定義
-- ============================================
-- Version: 1.0
-- Created: 2025-08-06
...
-- データベース作成（必要に応じて）
-- CREATE DATABASE csv_processing WITH ENCODING 'UTF8';
...
-- ============================================
-- 4. file_processing_results テーブル
-- ============================================
...
('chunk_size_records', '25', 'チャンク処理バッチサイズ（レコード数）', 'integer'),
('error_tolerance_percentage', '5', 'エラー許容率（％）', 'integer')
...
BEGIN
    -- 一括更新処理
    INSERT INTO user_statistics (user_id, login_count, post_count, last_updated, last_execution_id)
    SELECT 
        (value->>'user_id')::VARCHAR(10),
        (value->>'login_increment')::BIGINT,
        (value->>'post_increment')::BIGINT,
        CURRENT_TIMESTAMP,
        (value->>'execution_id')::VARCHAR(100)
...
COMMENT ON FUNCTION batch_update_user_statistics IS 'バッチユーザー統計更新プロシージャ';

-- 処理実行完了更新プロシージャ
CREATE OR REPLACE FUNCTION complete_processing_execution(
    p_execution_id VARCHAR(100),
    p_status VARCHAR(20),
    p_total_records INTEGER,
    p_success_records INTEGER,
    p_error_records INTEGER,
    p_execution_output JSONB DEFAULT NULL,
...
BEGIN
    ...
    SET
        ...
        processed_records = p_success_records + p_error_records,
        success_records = p_success_records,
        error_records = p_error_records,
        processing_time_seconds = EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - v_start_time)),
        execution_output = COALESCE(p_execution_output, execution_output),
...
COMMENT ON FUNCTION complete_processing_execution IS '処理実行完了更新プロシージャ';
...
-- CREATE ROLE application_role WITH LOGIN PASSWORD 'secure_password';
...
-- ============================================
-- 10. データアーカイブ用テーブル（オプション）
-- ============================================

-- 処理実行アーカイブテーブル
CREATE TABLE IF NOT EXISTS processing_executions_archive (
...
COMMENT ON FUNCTION archive_old_executions IS 'データアーカイブ処理関数';
